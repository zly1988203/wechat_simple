...
$(function() {
    ...
    var initCityTimePicker = function (_input) {
        var config = {};
        var input = _input;
        var _tips = '';
        ...
        for(var i=0; i < 5; i++) {//i 可选择的天数
            dates.push({value:getDate(d).value, text:getDate(d).text});
            d.setDate(d.getDate()+1);
        }
        config.day = dates;
        ...
        // 默认值
        var d = getNow();
        config.data = {day:getDate(d).value, hour:getTime(d.getHours(), d.getMinutes()).value, minute:_tips};
        if(input.val()) {
        ...
        $.timePicker(config, function(values, texts) {
            if(texts.hour) {
                ...
                if(input.attr('id') == 'present_startTime') {
                    showSharingCarPrice();
                } else {
                    showCharterCarType();
                }
