...
var adrsUtils = {
    ...
    selectAddress: function (param,callback) {
        ...
        popup_main.find('.serach-input input').on('input', function() {
            var self = $(this);
            ...
            $('#searchResult').html(strHtml).children('li').off(adrsUtils.targetType).on(adrsUtils.targetType, function() {
        ...
        /*点击历史记录返回数据*/
        popup_main.find('.wrapper').find('.history span').on(adrsUtils.targetType, function () {
            var data = $(this).text();
    ...
    //     /*点击当前城市数据*/
    //     $('#searchWrapper .current-city ul li').on('click', function () {
    //         var data = $(this).text();
    //         $('#search-address').setPopupData(data);
    //         $('#search-address .cancel').triggerHandler('click');
    //     });
